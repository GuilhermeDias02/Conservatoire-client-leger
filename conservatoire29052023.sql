...
CREATE TABLE `heure` (`TRANCHE` char(32) NOT NULL) ENGINE = InnoDB DEFAULT CHARSET = latin1;
--
-- Déchargement des données de la table `heure`
--

INSERT INTO `heure` (`TRANCHE`)
VALUES ('10h-11h'),
    ('10h-12h'),
    ('11h-12h'),
    ('13h-14h'),
...
CREATE TABLE `instrument` (`LIBELLE` char(32) NOT NULL) ENGINE = InnoDB DEFAULT CHARSET = latin1;
--
-- Déchargement des données de la table `instrument`
--

INSERT INTO `instrument` (`LIBELLE`)
VALUES ('Accordéon'),
    ('Basse'),
    ('Batterie'),
    ('Flûte'),
...
CREATE TABLE `jour` (`JOUR` char(32) NOT NULL) ENGINE = InnoDB DEFAULT CHARSET = latin1;
--
-- Déchargement des données de la table `jour`
--

INSERT INTO `jour` (`JOUR`)
VALUES ('jeudi'),
    ('lundi'),
    ('mardi'),
    ('mercredi'),
...
INSERT INTO `login` (`id`, `indentifiant`, `mdp`)
VALUES (
        1,
        'GuilhermeAdmin',
    ...
    (
        2,
        'GuilhermeAdmin',
        '746ea4028f2c438b5798afdd9ae74479'
    );
-- --------------------------------------------------------
--
...
CREATE TABLE `niveau` (`NIVEAU` int(11) NOT NULL) ENGINE = InnoDB DEFAULT CHARSET = latin1;
--
...
VALUES (1, 4, 1, 'avril-juin', '2000-01-01', 0),
...
INSERT INTO `personne` (`ID`, `NOM`, `PRENOM`, `TEL`, `MAIL`, `ADRESSE`)
VALUES (
        1,
        'Dupont',
        'Edouard',
        '0123456789',
        'edouard.dupont@example.com',
        '12 rue de la Paix'
    ),
    (
        2,
    ...
    (
        ...
        'Thierry',
        '0901234567',
        'thierry.sauvage@example.com',
        '108 avenue des Ternes'
    ),
    (
        10,
        'Simon',
    ...
    (
        ...
        'Lucie',
        '0234567890',
        'lucie.tanguy@example.com',
        '132 avenue des Gobelins'
    ),
    (
        12,
        'Thomas',
        'Guillaume',
        '0345678901',
        'guillaume.thomas@example.com',
        '144 rue Saint-Antoine'
    ),
    (
        13,
        'Vidal',
    ...
    (
        ...
        'Alexandre',
        '0567890123',
        'alexandre.boucher@example.com',
        '168 avenue de Clichy'
    ),
    (
        15,
        'Chevalier',
...
INSERT INTO `trim` (`LIBELLE`, `DATEFIN`)
VALUES ('avril-juin', '2023-06-21'),
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`localhost` SQL SECURITY DEFINER VIEW `veleve` AS
SELECT `personne`.`ID` AS `id`,
    `personne`.`NOM` AS `nom`,
    `personne`.`PRENOM` AS `prenom`,
    `personne`.`TEL` AS `tel`,
    `personne`.`MAIL` AS `mail`,
    `personne`.`ADRESSE` AS `adresse`,
    `eleve`.`NIVEAU` AS `niveau`,
    `eleve`.`BOURSE` AS `bourse`
FROM (
        `personne`
        join `eleve` on(`personne`.`ID` = `eleve`.`IDELEVE`)
    );
-- --------------------------------------------------------
--
...
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`localhost` SQL SECURITY DEFINER VIEW `vprof` AS
SELECT `personne`.`ID` AS `id`,
    `personne`.`NOM` AS `nom`,
    `personne`.`PRENOM` AS `prenom`,
    `personne`.`TEL` AS `tel`,
    `personne`.`MAIL` AS `mail`,
    `personne`.`ADRESSE` AS `adresse`,
    `prof`.`INSTRUMENT` AS `instrument`,
    `prof`.`SALAIRE` AS `salaire`
FROM (
        `personne`
        join `prof` on(`personne`.`ID` = `prof`.`IDPROF`)
    );
--
...
--
ALTER TABLE `seance`
MODIFY `NUMSEANCE` int(11) NOT NULL AUTO_INCREMENT,
    AUTO_INCREMENT = 5;
--
